...
def model_configuration_page():
    ...
    train_df['result'] = (train_df['result'] > 0.5).astype(int)
    test_df['result'] = (test_df['result'] > 0.5).astype(int)
    ...
    with col1:
        ...
        if model_option in ["RNN", "CNN"]:
            ...
            batch_size = st.slider("배치 크기", min_value=1, max_value=256, value=32)
        else:
            epochs = None
            batch_size = None
...
def preprocess_and_select_features(train_df, test_df, scaling_option, feature_selection_option, pca_components=None, autoencoder_params=None):
    ...
    if scaling_option == "StandardScalerg":
    ...
    X_train = train_df.drop(columns=['result']).values
    ...
    elif feature_selection_option == "NCA":
...
def anomaly_detection_page():
    ...
    # Ensure 'result' column is in the dataframe
    if 'result' not in train_df.columns:
        st.error("'result' 열이 훈련 데이터에 없습니다.")
        return
    
    if 'result' not in test_df.columns:
        st.error("'result' 열이 테스트 데이터에 없습니다.")
        return
